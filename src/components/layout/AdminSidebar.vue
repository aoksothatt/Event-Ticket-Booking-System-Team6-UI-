  ...
  <aside class="sticky top-0 flex h-screen w-64 shrink-0 flex-col justify-between border-r border-slate-200 bg-white px-4 py-5 shadow-sm overflow-y-auto dark:border-slate-700 dark:bg-slate-800">
    <div>
      ...
      <div class="mb-6 flex items-center gap-2 px-0">
        ...
        <div class="flex-1">
          <p class="text-sm font-bold leading-tight text-slate-900 dark:text-white">{{ settings.platformName }}</p>
          <p class="text-[11px] font-medium leading-tight text-primary">{{ t('admin') }}</p>
        </div>
        <LanguageSwitcher />
        <button
          type="button"
      ...
      <RouterLink
        to="/admin/events"
        class="mb-6 flex items-center justify-center gap-2 rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-primary-contrast shadow-sm transition-all hover:bg-primary-hover hover:shadow"
      >
